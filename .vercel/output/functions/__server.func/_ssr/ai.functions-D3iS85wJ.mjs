...
const getFollowUpQuestions_createServerFn_handler = createServerRpc({
  id: "3a1982be35adca2338b054bcd959f47c33abe8f4400d4ae0fdb6ebd7d5f96019",
  name: "getFollowUpQuestions",
  filename: "src/lib/ai.functions.ts"
}, (opts) => getFollowUpQuestions.__executeServer(opts));
const getFollowUpQuestions = createServerFn({
  ...
  profile: any(),
  rawMemory: string()
})).handler(getFollowUpQuestions_createServerFn_handler, async ({
  data
}) => {
...
${profileStr}`
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            isComplete: {
              type: "boolean",
              description: "True if profile has enough info to generate a good resume"
            },
            questions: {
              type: "array",
              items: {
                ...
                properties: {
                  ...
                  field: {
                    type: "string",
                    description: "Which profile field this fills, e.g. 'email', 'experience[0].achievements'"
                  },
                  question: {
                    type: "string",
                    description: "The friendly question to ask the user"
                  },
                  helperText: {
                    type: "string",
                  ...
                  options: {
                    ...
                    description: "Quick-pick options the user can tap. Empty for free text only."
                  },
                  placeholder: {
                    type: "string"
                  }
  ...
  const result = extractToolArgs(json);
  return result;
});
const patchProfileWithAnswers_createServerFn_handler = createServerRpc({
  id: "ab8d9e1d698d4300df9f0bf7927e37f0f327ce394c206fc5ca59eb833e126efa",
  name: "patchProfileWithAnswers",
  filename: "src/lib/ai.functions.ts"
}, (opts) => patchProfileWithAnswers.__executeServer(opts));
...
})).handler(patchProfileWithAnswers_createServerFn_handler, async ({
  data
}) => {
  const json = await callGateway({
    apiKey: data.apiKey,
...
const resumeSchema = {
  ...
  required: ["name", "title", "summary", "experience", "projects", "education", "skills", "skillItems", "certifications"]
};
...
const chatEditResume_createServerFn_handler = createServerRpc({
  id: "83a99bd00f312255f1fa4387e6d2db8731d02727defa44cfeeb9059d9edb73ec",
  name: "chatEditResume",
  filename: "src/lib/ai.functions.ts"
}, (opts) => chatEditResume.__executeServer(opts));
...
})).handler(chatEditResume_createServerFn_handler, async ({
  data
}) => {
  const json = await callGateway({
    apiKey: data.apiKey,
    messages: [{
      role: "system",
      content: "You are an elite resume editor and AI assistant. The user will provide their current resume data (in JSON) and a message detailing what changes they want. You must output the fully updated resume data reflecting these changes using the save_resume tool. You must also provide a brief reply confirming what you changed. If the user asks to raise, lower, reduce, fill, or change skill bars, dots, stars, or visual skill levels, update resume.skillItems with names aligned to resume.skills and levels from 1 to 5."
    }, {
      role: "user",
...
User Request: ${data.userMessage}`
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            reply: {
              type: "string",
              description: "A brief message (1-2 sentences) confirming the changes made to the user."
            }
          },
  ...
  const result = extractToolArgs(json);
  return {
    resume: optimizeResumeForOnePage(result.resume),
    reply: result.reply
  };
});
...
export {
  ...
  chatEditResume_createServerFn_handler,
  generateResume_createServerFn_handler,
  getFollowUpQuestions_createServerFn_handler,
  improveBullet_createServerFn_handler,
  parseMemory_createServerFn_handler,
  patchProfileWithAnswers_createServerFn_handler,
