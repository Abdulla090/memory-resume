...
import { u as useAppStore } from "./router-BXFogTK1.mjs";
import { t as toast } from "../_libs/sonner.mjs";
import "./index.mjs";
import "../_libs/seroval.mjs";
import { a as Sparkles, P as Paperclip, F as FileText, c as ArrowUp, C as CircleCheck, I as ImagePlus, d as SkipForward } from "../_libs/lucide-react.mjs";
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      if (isComplete || questions.length === 0) {
        addMsg({
          from: "ai",
          content: isKu ? `پرۆفایلەکە وەرگیرا، ${parsed.name || "بەڕێز"}! چ ڕۆڵێک دەکەیتە ئامانج بۆ ئەم سیڤییە؟` : `Profile captured, ${parsed.name || "there"}! What role are you targeting for this resume?`
        });
        setStage("builder");
      } else {
        addMsg({
          from: "ai",
          content: isKu ? `پرۆفایلەکەتم دەستکەوت، ${parsed.name || "بەڕێز"}! تەنها چەند پرسیارێکی خێرا بۆ ئەوەی سیڤییەکەت بەهێزتر بێت.` : `Got your profile, ${parsed.name || "there"}! Just a few quick questions to make your resume even stronger.`
        });
