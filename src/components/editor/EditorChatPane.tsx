...
export function EditorChatPane({
  ...
  return (
    <div className="relative flex flex-col flex-1 min-h-0 rounded-3xl bg-white/70 shadow-[0_8px_40px_-12px_rgba(15,23,42,0.15),0_0_0_1px_rgba(255,255,255,0.8)] backdrop-blur-sm lg:rounded-3xl max-lg:h-full max-lg:rounded-[28px] max-lg:border max-lg:border-slate-200/70 max-lg:overflow-hidden">
      ...
      {showHistory && history.length > 0 && (
        <div className="max-h-36 shrink-0 overflow-y-auto border-b border-slate-100 bg-slate-50/60 px-3 py-2 space-y-1">
          {history.map((h) => (
            <div
              key={h.id}
              className="group flex items-center justify-between gap-2 rounded-xl px-3 py-2 hover:bg-white transition-colors"
            >
            ...
            >
              ...
              <div className="flex flex-col gap-2 max-w-[82%]">
                ...
                {msg.role === "assistant" && msg.snapshotId && (
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => onRevert(msg.snapshotId!)}
      ...
      <div
        className="shrink-0 px-4 pb-2 flex gap-2 overflow-x-auto max-lg:px-3"
                    ...
                    {showTools && (
                      ...
                      >
                        <button
                          ...
                          className="flex items-center gap-3 px-3 py-2.5 rounded-[16px] hover:bg-slate-100/80 transition-colors text-left group"
                        >
                          <div className="size-8 rounded-full bg-emerald-100 flex items-center justify-center group-hover:bg-emerald-500 group-hover:text-white transition-colors text-emerald-600">
                            <CheckCircle2 className="size-4" />
                          </div>
                          <span className="text-[13px] font-semibold text-slate-700">
                            {isKu ? "پشکنینی ATS" : "ATS Score"}
                          </span>
                        </button>
                        <button
                          type="button"
                          ...
                          className="flex items-center gap-3 px-3 py-2.5 rounded-[16px] hover:bg-slate-100/80 transition-colors text-left group"
                        >
                          <div className="size-8 rounded-full bg-amber-100 flex items-center justify-center group-hover:bg-amber-500 group-hover:text-white transition-colors text-amber-600">
                            <Wand2 className="size-4" />
                          </div>
                          <span className="text-[13px] font-semibold text-slate-700">
                            {isKu ? "چاک بکە" : "Auto-Fix"}
                          </span>
                        </button>
                        <button
                          type="button"
                          ...
                          className="flex items-center gap-3 px-3 py-2.5 rounded-[16px] hover:bg-slate-100/80 transition-colors text-left group"
                        >
                          <div className="size-8 rounded-full bg-purple-100 flex items-center justify-center group-hover:bg-purple-500 group-hover:text-white transition-colors text-purple-600">
                            <LayoutTemplate className="size-4" />
                          </div>
                          <span className="text-[13px] font-semibold text-slate-700">
                            {isKu ? "نەخشە" : "Templates"}
                          </span>
                        </button>
                        <button
                          type="button"
                          ...
                          className="flex items-center gap-3 px-3 py-2.5 rounded-[16px] hover:bg-slate-100/80 transition-colors text-left group"
                        >
                          <div className="size-8 rounded-full bg-blue-100 flex items-center justify-center group-hover:bg-blue-500 group-hover:text-white transition-colors text-blue-600">
                            <FileText className="size-4" />
                          </div>
                          <span className="text-[13px] font-semibold text-slate-700">
                            {isKu ? "کەڤەر لێتەر" : "Cover Letter"}
                          </span>
                        </button>
                      </motion.div>
                    )}
