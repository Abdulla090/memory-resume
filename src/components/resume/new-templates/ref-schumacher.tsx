  ...
  return (
    <div dir={rtl ? "rtl" : "ltr"} className="bg-white px-[80px] py-[84px] font-sans text-[#161616]" style={{ minHeight: "1122px", width: "100%" }}>
      <header className="grid grid-cols-[230px_1fr_1fr] gap-14 ">
        <Editable path="name" value={c.name} as="h1" className="text-[42px] font-black leading-[0.86] tracking-tight rtl:tracking-normal break-words" />
        {[c.email, c.phone, c.location].filter(Boolean).slice(0, 2).map((item) => {
          const path =
            item === c.location ? "location" : item === c.email ? "email" : "phone";
          return (
            <div key={path} className="pt-7 text-[13px] font-bold leading-4 min-w-0">
              <div className="mb-3 h-[2px] w-8 bg-neutral-400" />
              <Editable path={path} value={item as string} as="span" className="break-words" />
            </div>
          );
        })}
      </header>
